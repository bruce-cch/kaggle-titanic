# -*- coding: utf-8 -*-
"""
Created on Mon May 27 16:50:10 2019
...
test_data = pd.read_csv("c:/pythonwork/test.csv")
...
for i in index_NaN_age :
    ...
    age_pred =test_data["Age"][((test_data['SibSp'] == test_data.iloc[i]["SibSp"])\
                       & (test_data['Parch'] == test_data.iloc[i]["Parch"]) \
                       & (test_data['Pclass'] == test_data.iloc[i]["Pclass"]))].median()
    if not np.isnan(age_pred) :
...
guess_Fare = train_data.Fare.loc[ (train_data.Ticket == '3') & \
                                  (train_data.Pclass == 3) & \
                                  (train_data.Embarked == 'S')].median()
train_data.Fare.fillna(guess_Fare , inplace=True)

guess_Fare = test_data.Fare.loc[ (test_data.Ticket == '3') & \
                                 (test_data.Pclass == 3) & \
                                 (test_data.Embarked == 'S')].median()
test_data.Fare.fillna(guess_Fare , inplace=True)

train_data['Fare-bin'] = pd.qcut( train_data.Fare, 5,
                                  labels=[ 1,2,3,4,5 ] ).astype(int)

#there's a NAN within DataFrame test_data
...
train_data = pd.concat([train_data, pd.get_dummies(train_data['Title'])],
                        axis=1).drop(labels=['Name'], axis=1)
test_data = pd.concat([test_data, pd.get_dummies(test_data['Title'])],
                       axis=1).drop(labels=['Name'], axis=1)
test_data.shape
...
X = train_data[["Pclass","Sex","Age","Family","Fare-bin","Embarked",
                "Ticket_1","Ticket_2","Ticket_3","Ticket_4","Ticket_C",
                "Ticket_P","Ticket_S","Master","Miss","Mr","Mrs","Others"]]
...
def model(X_train, Y_train, X_test, Y_test, learning_rate = 0.01,
    ...
    with tf.Session() as sess:
        ...
        plt.plot( np.squeeze(costs), marker='X', color='green' )
        plt.ylabel('cost')
